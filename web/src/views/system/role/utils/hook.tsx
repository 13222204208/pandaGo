...
import { getMenuSimpleList } from "@/api/menu";
import {
  getRoleList,
  createRole,
  updateRole,
...
export function useRole(treeRef: Ref) {
  ...
  function onChange({ row, index }) {
    ElMessageBox.confirm(
      ...
      .then(async () => {
        ...
        // 调用更新接口
        const res = await updateRole(row.id, {
          status: row.status,
          name: row.name,
          code: row.code,
          remark: row.remark
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          if (valid) {
            ...
            if (title === "新增") {
              // 调用新增角色接口
              const res = await createRole(curData);
              if (res.code === 0) {
                chores();
              } else {
                message(res.message, { type: "error" });
              ...
              // 调用修改角色接口
              const res = await updateRole(row.id, curData);
              if (res.code === 0) {
                chores();
              } else {
                message(res.message, { type: "error" });
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await saveRoleMenuIds(id, menuIds);
      if (res.code === 0) {
