...
// StartALL 启动所有任务
func StartALL(pandaCrontab []*entity.PandaCrontab) (err error) {
	crons.Lock()
	defer crons.Unlock()
	...
	for _, cron := range pandaCrontab {
		...
		if cron.Policy == consts.CronPolicyOnce || cron.Policy == consts.CronPolicyTimes {
			if _, err = dao.PandaCrontab.Ctx(gctx.GetInitCtx()).Where("id", cron.Id).Data(g.Map{"status": consts.StatusDisable, "updated_at": gtime.Now()}).Update(); err != nil {
...
// RefreshStatus 刷新状态
func RefreshStatus(pandaCrontab *entity.PandaCrontab) (err error) {
	if pandaCrontab == nil {
		return
	}

	if pandaCrontab.Status == consts.StatusEnabled {
		return ResetStart(pandaCrontab)
	}
	return Stop(pandaCrontab)
}

// Stop 停止单个任务
func Stop(pandaCrontab *entity.PandaCrontab) (err error) {
	cr := gcron.Search(GenCronSn(pandaCrontab))
	if cr == nil {
		return
	}
	cr.Stop()
...
// ResetStart 重置任务
func ResetStart(pandaCrontab *entity.PandaCrontab) (err error) {
	if err = Stop(pandaCrontab); err != nil {
		return
	}
	if err = Delete(pandaCrontab); err != nil {
		return
	}
	return Start(pandaCrontab)
}

// Once 立即执行一次某个任务
func Once(ctx context.Context, pandaCrontab *entity.PandaCrontab) error {
	crons.RLock()
	defer crons.RUnlock()

	for _, v := range crons.tasks {
		if v.Name == pandaCrontab.Name {
			utility.SafeGo(ctx, func(ctx context.Context) {
				v.Fun(GenCronCtx(pandaCrontab))
			})
			return nil
		}
	}
	return gerror.Newf("定时任务不存在：%+v", pandaCrontab.Name)
}

// Delete 删除任务
func Delete(pandaCrontab *entity.PandaCrontab) (err error) {
	if pandaCrontab == nil {
		return
	}

	for _, v := range gcron.Entries() {
		if v.Name == GenCronSn(pandaCrontab) {
			gcron.Remove(v.Name)
		}
	}
	return
...
// Start 启动单个任务
func Start(pandaCrontab *entity.PandaCrontab) (err error) {
	if pandaCrontab == nil {
		return
	}

	c := gcron.Search(GenCronSn(pandaCrontab))
	if c != nil {
		c.Start()
		return
	}
	return StartALL([]*entity.PandaCrontab{pandaCrontab})
}

// DispatchLog 查看指定任务的调度日志
func DispatchLog(pandaCrontab *entity.PandaCrontab) (log *Log, err error) {
	path := fmt.Sprintf("%v/%v", Logger().GetConfig().Path, GenCronSn(pandaCrontab))
